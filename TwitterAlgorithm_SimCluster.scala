# -*- coding: utf-8 -*-
"""
https://github.com/twitter/the-algorithm/blob/main/simclusters-ann/server/src/main/scala/com/twitter/simclustersann/candidate_source/ApproximateCosineSimilarity.scala
 ...
 * 2. Fetch top N clusters' top tweets from the ss(TopTweetsPerCluster index).
...
object ApproximateCosineSimilarity extends ApproximateCosineSimilarity {
  ...
  override def apply(
    ...
    // GSS: cluster of potential users that the end-user does not currently follow with potential that the 
    // end-user will be interested in following said users
